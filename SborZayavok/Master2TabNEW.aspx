<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Master2TabNEW.aspx.cs" Inherits="SborZayavok.Master2TabNEW" %>
...
<body>
    <form id="form1" runat="server">
        <div style="height: 954px; background-color: #F8F8FF;">
            <asp:Label ID="Label1" runat="server" Text="Страница Мастера№2 - Новые заявки"></asp:Label>
            <br />
            <br />
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:test1ConnectionString1 %>" DeleteCommand="DELETE FROM [spisokzayavok1] WHERE [НомерЗаявки] = @НомерЗаявки AND [ДатаЗаявки] = @ДатаЗаявки AND [ФИО] = @ФИО AND [ТабельныйНомер] = @ТабельныйНомер AND [НомерТелефона] = @НомерТелефона AND [ТипЗаявки] = @ТипЗаявки AND [ОписаниеЗаявки] = @ОписаниеЗаявки" InsertCommand="INSERT INTO [spisokzayavok1] ([НоваяЗаявка], [ДатаЗаявки], [ФИО], [ТабельныйНомер], [НомерТелефона], [ТипЗаявки], [ОписаниеЗаявки], [СтатусЗаявки], [ОписаниеСтатуса], [Мастер]) VALUES (@НоваяЗаявка, @ДатаЗаявки, @ФИО, @ТабельныйНомер, @НомерТелефона, @ТипЗаявки, @ОписаниеЗаявки, @СтатусЗаявки, @ОписаниеСтатуса, @Мастер)" SelectCommand="SELECT * FROM [spisokzayavok1] WHERE (([Мастер] = @Мастер) AND ([НоваяЗаявка] = @НоваяЗаявка))" UpdateCommand="UPDATE [spisokzayavok1] SET [НоваяЗаявка] = @НоваяЗаявка, [СтатусЗаявки] = @СтатусЗаявки, [ОписаниеСтатуса] = @ОписаниеСтатуса, [Мастер] = @Мастер WHERE [НомерЗаявки] = @НомерЗаявки AND [ДатаЗаявки] = @ДатаЗаявки AND [ФИО] = @ФИО AND [ТабельныйНомер] = @ТабельныйНомер AND [НомерТелефона] = @НомерТелефона AND [ТипЗаявки] = @ТипЗаявки AND [ОписаниеЗаявки] = @ОписаниеЗаявки">
                ...
                <SelectParameters>
                    <asp:Parameter DefaultValue="2" Name="Мастер" Type="Int32" />
